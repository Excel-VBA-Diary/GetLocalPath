...
このときフォルダーの命名規則は似て非なるものになります。通常、フォルダーの名前はサイト名とフォルダー名をハイフン（-）で連結した名前になりますが、「同期」はサイト名が左でフォルダー名が右、「OneDriveへのショートカットの追加」はフォルダー名が左でサイト名が右になります。左右が逆になるということです。  
また、生成されるローカルパスは次のとおり微妙に異なります。 
...
### /(2) 「OneDriveへのショートカットの追加」でマウントした後にフォルダー名を変更した場合  
